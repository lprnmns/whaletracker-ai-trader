...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze realized DEX trade PnL for wallets with Dune.")
    ...
    sql = build_sql(wallets, max(1, min(args.days, 365)), dec(args.min_trade_usd))
    rows = execute_dune_sql(api_key, sql, args.timeout, args.poll)
    ...
    (out_dir / "query.sql").write_text(sql, encoding="utf-8")
